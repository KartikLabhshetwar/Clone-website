export default function Navbar() {
    ...
    return (
        <nav>
            <div className="bg-black p-5 flex justify-between rounded-2xl m-10">
                <div className="flex items-center">
                    <img src="../../public/cursor.webp" className="h-8"/>
                    <a className="text-white h-13" href="/">
                            <svg className="h-[13px] text-brand-foreground lg:h-[17px]" fill="currentColor" viewBox="0 0 69 13" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className="flex gap-14">
                    {menuItems.slice(1, 6).map((item) => (
                        <a className="text-white font-mono font-semibold" key={item.name} href={item.link}>{item.name}</a>
                    ))}
                </div>
                <div className="flex gap-6">
                    {menuItems.slice(6).map((item) => (
                        <a className="text-white font-mono font-semibold border border-white rounded-lg px-3 py-1" key={item.name} href={item.link}>{item.name}</a>
                    ))}
                </div>
            </div>
        </nav>
